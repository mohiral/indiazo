...
const CrashGame = () => {
  ...
  const [customCrashValue, setCustomCrashValue] = useState(2.0) // Changed default to 2.0 for easier testing
  ...
  const [statusMessage, setStatusMessage] = useState("") // Added for status messages
  const [isLoading, setIsLoading] = useState(true) // Add loading state
  const [connectedUsers, setConnectedUsers] = useState([]) // Track connected users
  ...
  useEffect(() => {
    ...
    // Set a timeout to hide the loading state after assets are loaded
  ...
  useEffect(() => {
    ...
    socket.on("game_state", (state) => {
      ...
      setIsLoading(false) // Hide loading when game state is received
    })
    ...
    // NEW: Track connected users
    socket.on("users_update", (users) => {
      console.log("Connected users update:", users)
      setConnectedUsers(users)
    })

    // NEW: Handle new user connection
    socket.on("user_connected", (user) => {
      console.log("New user connected:", user)
      setConnectedUsers((prev) => [...prev, user])

      // Show notification for new user
      setStatusMessage(`${user.username || "A new player"} has joined!`)
    ...
    // NEW: Added message handlers for crash point setting
  ...
  const calculatePartialPath = () => {
    ...
    const y = (1 - t) * (1 - t) * startY + 2 * (1 - t) * t * controlY + t * t * endY // Corrected formula
  ...
  if (isLoading) {
    return (
      <div className="max-w-3xl mx-auto bg-gradient-to-b from-black to-blue-900 flex flex-col items-center justify-center text-white p-2 sm:p-4 relative min-h-[600px]">
        {/* Navigation Bar Skeleton */}
        <div className="w-full bg-gradient-to-r from-purple-600/50 to-blue-600/50 text-white text-center p-1 sm:p-2 rounded-t-lg font-bold text-sm sm:text-lg mb-1 shadow-lg animate-pulse">
          COSMIC CRASH
        </div>

        {/* Last Crashes Display Skeleton */}
        <div className="w-full bg-gray-900/70 mb-2 p-2 rounded flex flex-wrap justify-center gap-2 animate-pulse">
          <span className="text-cyan-300/50 text-sm">Recent Crashes:</span>
          {[...Array(5)].map((_, index) => (
            <div key={index} className="h-5 w-12 bg-gray-700/50 rounded"></div>
          ))}
        </div>

        {/* Graph Container Skeleton */}
        <div className="w-full h-auto min-h-[250px] md:min-h-[300px] bg-[#0a192f] p-2 sm:p-4 rounded-lg flex flex-col items-center relative border border-indigo-900/50 shadow-[0_0_15px_rgba(66,153,225,0.3)]">
          {/* Loading indicator */}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
              <Loader2 className="h-12 w-12 text-cyan-500 animate-spin" />
              <p className="text-cyan-300 text-lg font-medium">Loading game...</p>
            </div>
          </div>
          ...
          <svg className="absolute w-full h-full opacity-30">
            <path
              d={`M 0 ${window.innerWidth < 640 ? 160 : 240} Q ${window.innerWidth < 640 ? 150 : 300} ${window.innerWidth < 640 ? 100 : 160} ${window.innerWidth < 640 ? 300 : 600} 0`}
          ...
          {/* Rocket Skeleton */}
          <div className="absolute bottom-[160px] left-0 w-16 h-16 sm:w-24 sm:h-24 bg-gray-700/30 rounded-full animate-pulse"></div>
        </div>

        {/* Bet Section Skeleton */}
        <div className="w-full mt-2 min-h-[200px] bg-gray-900/50 rounded-lg p-4 animate-pulse">
          <div className="h-8 w-32 bg-gray-700/50 rounded mb-4"></div>
          <div className="h-12 w-full bg-gray-700/50 rounded mb-4"></div>
          <div className="h-10 w-24 bg-blue-700/50 rounded mx-auto"></div>
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      {/* Connected Users Display - NEW */}
      {/* <div className="w-full bg-gray-900/70 mb-2 p-2 rounded">
        <div className="flex items-center justify-between">
          <span className="text-cyan-300 text-sm">Players Online: {connectedUsers.length}</span>
          <div className="flex -space-x-2">
            {connectedUsers.slice(0, 5).map((user, index) => (
              <div
                key={index}
                className="w-6 h-6 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center text-xs border border-gray-800"
                title={user.username || `Player ${index + 1}`}
              >
                {(user.username || "P")[0].toUpperCase()}
              </div>
            ))}
            {connectedUsers.length > 5 && (
              <div className="w-6 h-6 rounded-full bg-gray-700 flex items-center justify-center text-xs border border-gray-800">
                +{connectedUsers.length - 5}
              </div>
            )}
          </div>
