...
const CrashGame = () => {
  ...
  useEffect(() => {
    ...
    // No cleanup needed as we want the game to run continuously
  }, [])
  ...
  const handleCrash = () => {
    ...
    setTimeout(() => {
      let count = 5
      setCountdown(count)
      setGameState("waiting") // Set game state to waiting to allow new bets
  ...
  return (
    <div className="min-h-screen max-w-3xl min-w-3xl bg-gradient-to-b from-black to-blue-900 flex flex-col items-center justify-center text-white p-4 relative">
      {/* Navigation Bar */}
        ...
        <div className="absolute top-2 left-4 z-10">
          <span
            className={`px-3 py-1 rounded text-sm font-bold ${
              gameState === "waiting" ? "bg-yellow-600" : gameState === "active" ? "bg-green-600" : "bg-red-600"
            }`}
          >
        ...
        {gameState === "active" ? (
          ...
          <div className="z-10 mt-8 flex flex-col items-center">
            <div className="text-7xl font-bold text-white mb-2">
              {gameState === "crashed" ? "CRASHED" : "NEXT ROUND"}
            </div>
