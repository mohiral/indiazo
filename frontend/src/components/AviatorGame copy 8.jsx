...
// Preload and optimize rocket image
const rocketImage = new Image()
rocketImage.src = "/rocket.webp" // Convert GIF to WebP for better performance

const CrashGame = () => {
  const [multiplier, setMultiplier] = useState(1.0)
  const [crashed, setCrashed] = useState(false)
  const [customCrashValue, setCustomCrashValue] = useState(2.0)
  const [crashSequenceInput, setCrashSequenceInput] = useState("")
  ...
  // Animation frame reference for smoother animations
  const animationFrameRef = useRef(null)
  const lastTimestampRef = useRef(0)

  // Memoize path coordinates for better performance
  const pathCoordinates = useMemo(() => getPathCoordinates(), [])
  ...
  // Socket connection and event listeners - optimized with useCallback
  useEffect(() => {
    // Connection status
    const handleConnect = () => {
    ...
    const handleMultiplierUpdate = (newMultiplier) => {
      ...
      const estimatedProgress = Math.min((newMultiplier - 1) / 0.8, 1) // Reduced divisor for faster movement
    ...
    const handleGameCrashed = (crashPoint) => {
    ...
    const handleLastCrashes = (crashes) => {
      console.log("Last crashes:", crashes)
      setLastCrashes(crashes)
    }

    // Bet handling
    const handleNewBet = (betInfo) => {
    ...
    // Cashout handling
    const handlePlayerCashedOut = (cashout) => {
      console.log("Player cashed out:", cashout)
    }

    const handleCashoutConfirmed = (cashout) => {
    ...
    socket.on("last_crashes", handleLastCrashes)
    socket.on("new_bet", handleNewBet)
    socket.on("bet_confirmed", handleBetConfirmed)
    socket.on("bet_error", handleBetError)
    socket.on("player_cashed_out", handlePlayerCashedOut)
    socket.on("cashout_confirmed", handleCashoutConfirmed)
    ...
    return () => {
      ...
      socket.off("last_crashes", handleLastCrashes)
      socket.off("new_bet", handleNewBet)
      socket.off("bet_confirmed", handleBetConfirmed)
      socket.off("bet_error", handleBetError)
      socket.off("player_cashed_out", handlePlayerCashedOut)
      socket.off("cashout_confirmed", handleCashoutConfirmed)
  ...
  // Handle rocket animation with requestAnimationFrame for smoother animation
  useEffect(() => {
    if (!rocketAtCorner || crashed) return

    const animateDotPosition = (timestamp) => {
      ...
      animationFrameRef.current = requestAnimationFrame(animateDotPosition)
    }
  ...
  // Handle window resize for responsive paths - debounced
  useEffect(() => {
    let resizeTimeout
  ...
  // Animate smoke particles with requestAnimationFrame
  useEffect(() => {
  ...
  const handleCrash = useCallback(() => {
    ...
    // Generate rocket pieces for the explosion - reduced number for better performance
    const pieces = []
    for (let i = 0; i < 8; i++) {
      // Reduced from 12 to 8 pieces
    ...
    // Create smoke particles - reduced number for better performance
    const smokeParticles = []
    for (let i = 0; i < 10; i++) {
      // Reduced from 15 to 10 particles
      smokeParticles.push({
        id: i,
        x: rocketPosition.x,
        y: rocketPosition.y,
    ...
    setUserBet(null)
    setBetConfirmed(false)
  }, [rocketPosition, crashDirection])

  // Animate rocket pieces with requestAnimationFrame
  useEffect(() => {
  ...
  // Function to handle setting crash sequences
  const handleSetCrashSequence = useCallback(() => {
    // Parse the comma-separated string into an array of numbers
    const values = crashSequenceInput
      .split(",")
      .map((val) => Number.parseFloat(val.trim()))
      .filter((val) => !isNaN(val))

    if (values.length === 0) {
      setStatusMessage("Please enter valid crash values separated by commas")
      setTimeout(() => setStatusMessage(""), 5000)
      return
    }

    socket.emit("set_crash_sequence", { values })
    setShowSequenceInput(false)
  }, [crashSequenceInput])

  // Function to get path coordinates - memoized
  ...
  // Calculate the path for the current progress - optimized
  const calculatePartialPath = useCallback(() => {
    const { startY, controlX, controlY, endX, endY } = pathCoordinates
  ...
  }, [pathProgress, pathCoordinates])

  // Calculate the fill path for the current progress - optimized
  const calculatePartialFillPath = useCallback(() => {
    const { startY, controlX, controlY, endX, endY } = pathCoordinates
  ...
  // Get the current position of the rocket for crash animation - optimized
  ...
  // Memoize the stars and planets to prevent re-renders
  ...
  // Memoize the last crashes display
  const lastCrashesDisplay = useMemo(
    () => (
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-auto min-h-[250px] md:min-h-[300px] bg-[#0a192f] p-2 sm:p-4 rounded-lg flex flex-col items-center relative border border-indigo-900 shadow-[0_0_15px_rgba(66,153,225,0.5)]">
        ...
        {/* Enhanced Stars Background - Memoized */}
        {spaceBackground}
        ...
        <style jsx>{`
          ...
          /* Shooting stars - optimized with will-change */
          .shooting-stars {
            position: absolute;
            width: 100%;
            height: 100%;
          ...
          /* Enhanced Orbit Animations - optimized with will-change */
          @keyframes orbit {
          ...
          /* Enhanced Planets - optimized with will-change */
          ...
          /* Screen shake animation - optimized with will-change */
          ...
          /* Enhanced crash inertia animation - optimized with will-change */
                ...
                <div className="mb-2 flex flex-col items-center">
                  {!showSequenceInput ? (
                    <div className="flex items-center">
                      ...
                      <button
                        onClick={() => setShowSequenceInput(true)}
                        className="ml-2 bg-purple-600 hover:bg-purple-700 text-white px-2 py-1 rounded text-sm"
                      >
                        Sequence
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full max-w-xs">
                      <div className="flex items-center w-full mb-1">
                        <input
                          type="text"
                          value={crashSequenceInput}
                          onChange={(e) => setCrashSequenceInput(e.target.value)}
                          className="flex-1 bg-gray-800 text-white px-2 py-1 rounded-l border border-gray-700 text-center"
                          placeholder="2.0, 5.0, 7.0"
                        />
                        <button
                          onClick={handleSetCrashSequence}
                          className="bg-purple-600 hover:bg-purple-700 text-white px-2 py-1 rounded-r border border-purple-700 text-sm"
                        >
                          Set
                        </button>
                      </div>
                      <div className="text-xs text-gray-300 mb-1">Enter comma-separated crash values</div>
                      <button
                        onClick={() => setShowSequenceInput(false)}
        ...
        <div className="relative w-full h-48 sm:h-64 mt-2 overflow-visible">
          ...
          {/* Path and Background Combined - Using memoized path calculations */}
          ...
          {/* Enhanced Rocket with Cinematic Slow Motion Crash - Using WebP instead of GIF */}
          {(gameActive || crashed) && !explosionActive && (
            ...
            >
              <div className="relative">
                ...
                {/* Rocket image with WebP format instead of GIF */}
                {rocketLoaded ? (
                  <img
                    ref={rocketRef}
                    src="/rocket.webp" // Using WebP instead of GIF
                    alt="Rocket"
          ...
          {/* Rocket Pieces after explosion - Reduced number for better performance */}
          ...
          {/* Smoke particles that follow the plane - Reduced number for better performance */}
        ...
        {explosionActive && (
          <>
            ...
            >
              ...
              {/* Explosion particles - Reduced number for better performance */}
              <motion.div
              ...
              >
                {[...Array(24)].map(
                  (
                    _,
                    i, // Reduced from 36 to 24 particles
                  ) => (
                    <motion.div
                      key={i}
                      ...
                      initial={{ x: 0, y: 0 }}
                      animate={{
                        x: Math.cos(i * 15 * (Math.PI / 180)) * (80 + Math.random() * 60), // Increased angle spacing
                        y: Math.sin(i * 15 * (Math.PI / 180)) * (80 + Math.random() * 60),
                        opacity: [1, 0],
                        scale: [1, 0.5],
                      }}
      ...
      {/* Add global styles for animations - Optimized with will-change */}
      <style jsx global>{`
        @keyframes flame {
      ...
      {/* Hidden audio elements for sound effects - Preloaded */}
