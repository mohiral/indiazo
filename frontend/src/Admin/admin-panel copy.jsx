                        ...
                        <tbody className="divide-y divide-gray-700">
                          {gameDetails.bets.map((bet, index) => (
                            <tr key={index}>
                              ...
                              <td className="px-4 py-2 whitespace-nowrap">
                                <span
                                  className={`font-medium ${bet.status === "won" ? "text-green-400" : "text-red-400"}`}
                                >
                                  {bet.status === "won" ? `Cashed out at ${bet.cashoutMultiplier.toFixed(2)}x` : "Lost"}
                                </span>
                              </td>
                              <td className="px-4 py-2 whitespace-nowrap">
                ...
                <tbody className="divide-y divide-gray-700">
                  {games.map((game, index) => (
                    <tr key={index} className="hover:bg-gray-700">
                      <td className="px-4 py-2 whitespace-nowrap text-sm">{formatDate(game.timestamp)}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm">{game.gameId.substring(0, 8)}...</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span className={`font-medium ${game.crashPoint < 2 ? "text-red-400" : "text-green-400"}`}>
                          {game.crashPoint.toFixed(2)}x
                        </span>
                      ...
                      <td className="px-4 py-2 whitespace-nowrap text-sm">{game.playerCount}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm">{formatCurrency(game.totalBetAmount)}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm">{formatCurrency(game.totalWinAmount)}</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span className={game.adminProfit >= 0 ? "text-green-400" : "text-red-400"}>
                          {game.adminProfit >= 0 ? "+" : "-"}
                          {formatCurrency(Math.abs(game.adminProfit))}
